        ...
        overlay.querySelector('#p-import').onclick = () => {
            ...
            if (text) {
                ...
                for (let i = 0; i < rawLines.length; i++) {
                    ...
                    // Smart Parser: Check for Title + Content block pattern
                    // Title criteria: Short (< 50 chars), no sentence ending punctuation (。|；|;), and next line exists
                    const isPotentialTitle = line.length < 50 && !/[。；;]/.test(line);

                    if (isPotentialTitle && i + 1 < rawLines.length) {
                        const nextLine = rawLines[i + 1];
                        // Combine as Name + Content
                        results.push({ name: line, content: nextLine });
                        i++; // Skip next line as we consumed it
                    } else {
                        // Fallback: Check for Tab separation or treat as simple content
                        const parts = line.split(/\t/);
                        if (parts.length >= 2) {
                            results.push({ name: parts[0], content: parts.slice(1).join(' ') });
                        } else {
                            // Just content -> generate short name
                            let name = line.slice(0, 30);
                            if (line.length > 30) name += '...';
                            results.push({ name: name, content: line });
                        }
                    }
                }
                cb(results);
