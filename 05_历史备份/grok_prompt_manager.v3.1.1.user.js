...
(function () {
    ...
    /**
     * AutoRetryManager: Handles automatic retry logic for failed prompts
     */
    class AutoRetryManager {
        ...
        init() {
            ...
            console.log('[GPM] AutoRetryManager Initialized');
        }
        ...
        getModerationToasts() {
            return this.deepQueryAll('li[data-sonner-toast][data-type="error"][data-visible="true"]');
        }

        attachPromptListeners() {
            const textarea = this.deepQueryAll("textarea")[0];
            if (!textarea) return;
            if (textarea.dataset.gpmListenerAttached === "1") return;

            textarea.dataset.gpmListenerAttached = "1";
        ...
        checkModerationPopup() {
            ...
            if (this.retryCount >= this.maxRetryLimit) return;

            const toasts = this.getModerationToasts();
            if (!toasts.length) return;

            console.log("[GPM] ⚠️ Moderation toast detected");
            if (this.fixPrompt) {
                this.fixPromptBeforeRedo();
            }
            this.clickRedo();
        }
        ...
        fixPromptBeforeRedo() {
            ...
            if (!textarea.value.trim() && this.lastTypedPrompt) {
            ...
            const wordMap = {
                "nude": "wearing swimsuit", "naked": "partially clothed", "undress": "change outfit",
                "strip": "remove coat", "sex": "romantic", "sexual": "romantic", "erotic": "sensual pose",
                "sensual": "graceful", "seduce": "charm", "porn": "adult-themed", "boobs": "chest",
                "nipples": "chest area", "vagina": "body", "penis": "body", "butt": "backside",
                "anal": "rear", "cum": "fluid", "masturbate": "self-touch", "blowjob": "oral gesture",
                "threesome": "group", "kill": "defeat", "murder": "eliminate", "bloody": "dramatic",
                "blood": "red liquid", "gore": "dramatic", "violent": "intense", "torture": "challenge",
                "stab": "pierce", "shoot": "launch", "gunshot": "sound", "strangle": "hold tightly",
                "choke": "grip", "dead": "fallen", "death": "end", "corpse": "body", "suicide": "self-harm scene",
                "explosion": "burst", "bomb": "explosive object", "drug": "substance", "cocaine": "white powder",
                "heroin": "powder", "meth": "crystals", "marijuana": "plant", "weed": "plant", "joint": "rolled leaf"
            };

            Object.keys(wordMap).forEach((bad) => {
                const re = new RegExp("\\b" + bad + "\\b", "gi");
                txt = txt.replace(re, wordMap[bad]);
            });
        ...
        clickRedo() {
            ...
            const textarea = this.deepQueryAll("textarea")[0];
            let container = textarea ? textarea.closest("div") : null;
            let redoBtn = null;

            if (container) {
                // Try finding standard buttons or "Again" buttons
                const buttons = container.querySelectorAll("button");
                for (const btn of buttons) {
                    const label = (btn.getAttribute("aria-label") || "").toLowerCase();
                    if (label.includes("option")) continue;
                    if (btn.className.includes("bg-button-filled") || btn.dataset.slot === "button") {
                        redoBtn = btn;
                        break;
                    }
                }
                if (!redoBtn) redoBtn = buttons[buttons.length - 1];
            }

            if (!redoBtn) {
                console.log("[GPM] ❌ Redo button not found");
                this.isRetrying = false;
                this.autoRedo = false; // Safety stop
                this.renderPanel();
                return;
            }

            console.log("[GPM] 🔄 Clicking Redo...");
            setTimeout(() => {
                redoBtn.click();
                this.retryCount++;
                this.renderPanel();
        ...
        renderPanel() {
            ...
            // Only show if we are in an image view
            if (!this.currentImageId || location.pathname.includes('/imagine/favorites')) {
                this.panel = null;
                return;
            }

            this.panel = document.createElement('div');
            this.panel.className = 'gpm-panel'; // Use GPM class
            Object.assign(this.panel.style, {
                position: 'fixed',
                top: '80px', // Adjusted to not overlap top bar
                right: '20px',
                padding: '12px',
                zIndex: '999999',
                width: '220px',
            ...
            this.panel.innerHTML = `
                <div style="margin-bottom:8px; font-weight:bold; display:flex; justify-content:space-between;">
                    <span>Auto Redo</span>
                    <span>${this.retryCount} / ${this.maxRetryLimit}</span>
                </div>
                ...
                <div style="display:flex; align-items:center; gap:8px; opacity:0.8;">
                    Max Retries: 
                    <input type="number" id="gpm-input-max" min="1" max="50" value="${this.maxRetryLimit}" 
                    style="width:50px; background:rgba(255,255,255,0.1); border:1px solid rgba(255,255,255,0.2); color:white; border-radius:4px; padding:2px 4px;">
                </div>
            `;
            ...
            this.panel.querySelector('#gpm-input-max').onchange = (e) => {
                ...
                ">\r
                    ...
                    ">
                        ...
                        <div class="lib-row" style="display: flex; gap: 6px; align-items: center;">
                            ...
                            <button class="gpm-btn rename-lib-btn" title="重命名库 (Rename Library)">${ICON_SET.Rename}</button>
    ...
    `);
        ...
        setup(libraryData, libraries, onPromptClick, onLibChange, onImport, onExport, onAddLib, onDeleteLib, onAddPrompt, onExportAll, onDraftToggle, onPromptAction, onReorder, onAiAssist, onRenameLib) {
            ...
            // ✨ Right-click to rename current library
            select.oncontextmenu = (e) => {
                e.preventDefault();
                if (onRenameLib) onRenameLib();
            };
            ...
            const renBtn = this.shadow.querySelector('.rename-lib-btn');
            if (renBtn && onRenameLib) renBtn.onclick = onRenameLib;
...
Breast squeeze, pressing breasts together"></textarea>
    ...
    class App {
        ...
        init() {
            ...
            // ✨ SAFETY: Wait for Body if missing
            if (!document.body) {
                console.log('[GPM] App init waiting for body...');
            ...
            try {
                ...
                console.error('[GPM] AutoRetryManager init failed:', e);
            }
            ...
            // 🟢 Restore Icon: Render immediately after checks
            try {
                this.renderToggle();
            } catch (e) {
                console.error('[GPM] RenderToggle failed:', e);
            }
            ...
            try {
                ...
                console.error('[GPM] LoadLibraryData failed:', e);
            }
    ...
    window.showGPMPasteImport = function (cb) {
        ...
        overlay.innerHTML = `
            <div style="width:600px;background:#202020;border:1px solid rgba(255,255,255,0.2);border-radius:12px;padding:20px;color:white;display:flex;flex-direction:column;gap:15px;box-shadow:0 10px 40px rgba(0,0,0,0.5);">
                ...
                <div style="color:#888;font-size:12px;">请遵循 “标题 + 双换行 + 内容” 的格式 复制到文本框</div>
        ...
        overlay.querySelector('#p-import').onclick = () => {
            ...
            if (text) {
                let prompts = [];
                // Strategy: Check if user uses Double Newlines to separate items (Block Format)
                // e.g. "Title\n\nContent\n\nTitle2" or "Title\nContent\n\nTitle2"
                const blocks = text.split(/\n\s*\n/).filter(b => b.trim());

                if (blocks.length > 0 && blocks.some(b => b.trim().split('\n').length >= 2)) {
                    // Block Mode detected
                    prompts = blocks.map(b => {
                        const lines = b.trim().split('\n').filter(l => l.trim());
                        if (lines.length >= 2) {
                            // Line 1: Title, Rest: Content
                            ...
                            let name = parts[0];
                            if (name.length > 30) name = l.slice(0, 30) + '...';
                            return { name: name, content: l };
                        }
                    });
                } else {
                    // Line Mode (Legacy)
