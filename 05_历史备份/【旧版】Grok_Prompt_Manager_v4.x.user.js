...
(function () {
    ...
    const ICON_SET = {
        ...
        Menu: `<svg class="gpm-svg-icon" viewBox="0 0 24 24"><line x1="3" y1="12" x2="21" y2="12" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><line x1="3" y1="6" x2="21" y2="6" stroke="currentColor" stroke-width="2" stroke-linecap="round"/><line x1="3" y1="18" x2="21" y2="18" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>`
    };
...
Breast squeeze, pressing breasts together"></textarea>
    ...
    window.gpmBackupAll = function () {
        try {
            const app = window.grokPromptManagerApp;
            if (app && app.exportAllLibraries) {
                app.exportAllLibraries();
            } else {
                alert('备份功能未就绪 (Backup not ready)');
            }
        } catch (e) {
...
(注意：必须使用【 】作为标题分隔符，正文必须以"主体:"开头)`;
        ...
        overlay.querySelector('#p-import').onclick = () => {
            ...
            if (text) {
                ...
                // 🔍 FORMAT DETECTION: Try to detect input format
                let parsedAsJSON = false;

                // Try parsing as JSON first (for legacy format compatibility)
                try {
                    const jsonData = JSON.parse(normalized);
                    if (Array.isArray(jsonData)) {
                        // ✅ Valid JSON Array - Convert to new format
                ...
                // If not JSON, try Tab-delimited format
                if (!parsedAsJSON && normalized.includes('\t')) {
                    ...
                    if (results.length > 0) parsedAsJSON = true; // Mark as parsed
                }

                // If still not parsed, try 【Title】 format
                if (!parsedAsJSON) {
                    // 🛠️ STRICT MODE: ONLY ACCEPT 【Title】
                    // Matches 【Title】 followed by Content until next 【 or End
                    const regex = /【([^】]+)】([\s\S]*?)(?=【|$)/g;
                    let match;
                    while ((match = regex.exec(normalized)) !== null) {
                        const title = match[1].trim();
                        let content = match[2].trim();
                         // Sanity check: prevent empty titles
                        if (title) {
                            results.push({ name: title, content: content || title });
                        }
                    }
                }
