...
(function () {
    ...
    // ========================================
    // 🤖 AUTOMATION SERVICE (Infinite Loop Core)
    // ========================================
    class AutomationService {
        ...
        // --- Human Simulation ---
        async typeHumanly(element, text) {
            element.focus();
            await this.wait(50);
            ...
            for (let i = 0; i < text.length; i++) {
                if (!this.isRunning) break; // Emergency Stop check
                const char = text[i];
        ...
        async extractLastFrame(videoUrl) {
            console.log('[Auto] Fetching video for frame extraction:', videoUrl);
        ...
        async uploadImage(blob) {
            ...
            if (!fileInput) {
                ...
                const uploadBtn = Array.from(document.querySelectorAll('button')).find(b => {
                    ...
                    return l.includes('upload') || l.includes('add image') || l.includes('media');
                });
                if (uploadBtn) {
                    uploadBtn.click();
                    await this.wait(500);
            ...
            const file = new File([blob], "loop_frame.jpg", { type: "image/jpeg" });
            const dt = new DataTransfer();
            dt.items.add(file);
            fileInput.files = dt.files;
            fileInput.dispatchEvent(new Event('change', { bubbles: true }));
            fileInput.dispatchEvent(new Event('input', { bubbles: true }));
            await this.wait(1000); // Wait for upload
        }

        // --- Core: Upscale ---
        async triggerUpscale(videoSrc) {
            console.log('[Auto] Attempting Upscale...');

            // 1. Find reference video context
            const videoEl = Array.from(document.querySelectorAll('video')).find(v => v.src === videoSrc);
            const searchContext = videoEl ?
                (videoEl.closest('article') || videoEl.closest('.group') || document.body) : document.body;

            // 2. Find Button (Heuristic: "Upscale", "Enhance", "U1", "提升", "画质")
            let upscaleBtn = Array.from(searchContext.querySelectorAll('button')).find(b => {
                const label = (b.ariaLabel || b.innerText || b.title || '').toLowerCase();
                return label.includes('upscale') || label.includes('enhance') || label.includes('提升') || label.includes('画质');
            });

            if (!upscaleBtn) {
                console.warn('[Auto] Upscale button not found in context, scanning global...');
                upscaleBtn = Array.from(document.querySelectorAll('button')).reverse().find(b => { // Check newest buttons
                    const label = (b.ariaLabel || b.innerText || b.title || '').toLowerCase();
                    return label.includes('upscale') || label.includes('enhance') || label.includes('提升') || label.includes('画质');
                });
            }
            ...
            if (!upscaleBtn) {
                console.warn('[Auto] No upscale button found.');
                return null;
            }

            // 3. Click & Wait
            console.log('[Auto] Clicking Upscale:', upscaleBtn);
            upscaleBtn.click();
            await this.wait(2000); // Wait for request to register

            // 4. Wait for result (Recycle waitForNewVideo)
            // We pass the *current* video URL as "existing" to ignore it.
            const knownUrls = new Set(Array.from(document.querySelectorAll('video')).map(v => v.src));
            return this.waitForNewVideo(knownUrls, 180000); // 3 mins timeout for upscale (can be slower)
        }

        // --- Core: Wait for Video ---
        async waitForNewVideo(existingUrls, timeoutMs = 120000) {
            console.log('[Auto] Waiting for new video...');
            return new Promise((resolve, reject) => {
                ...
                const check = () => {
                    ...
                    for (let v of videos) {
                        if (v.src && !existingUrls.has(v.src) && v.src.startsWith('http')) {
                            // Filter valid video urls (avoid blob previews if possible, or handle them)
                            // Grok usually gives permanent URLs quickly.
                            if (v.videoWidth > 0) { // Ensure it has metadata
                                resolve(v.src);
                                return;
                            }
                        }
    ...
    (function () {
        ...
        // ========================================
        // 🔥 Early Fetch Hook (Must execute first)
        // ========================================
        // Expose store globally for Inspector to access
        ...
        window.fetch = async function (...args) {
            ...
            try {
                ...
                // Match the specific API endpoint for posts
                if (url.includes('/rest/media/post/list')) {
                    // Clone the response to read it without consuming the stream
                    res.clone().json().then(data => {
                        if (data.posts && data.posts.length > 0) {
                            data.posts.forEach(post => {
                                // Store by ID (handle both original and current IDs)
                                const key = post.originalPostId || post.id;
                                GLOBAL_POST_STORE[key] = post;
                            });
                            console.log(`[Inspector Hook] Cached ${data.posts.length} posts, Total: ${Object.keys(GLOBAL_POST_STORE).length}`);
                        }
                    }).catch(e => {
                        console.warn('[Inspector Hook] Parse data failed:', e);
                    });
                }
            } catch (e) {
                console.warn('[Inspector Hook] Fetch hook error:', e);
            }

            return res;
        };

        console.log('[Inspector Hook] Fetch intercepted, waiting for API requests...');
        ...
        const ICON_SET = {
            ...
            // AI Assist (Grok Monkey)
            AiAssist: `<svg class="gpm-svg-icon gpm-svg-anim" viewBox="0 0 24 24" style="color: #f58e0a;">
            <circle cx="12" cy="12" r="9" fill="none" stroke="currentColor" stroke-width="2"/>
            <circle cx="3" cy="12" r="2" fill="currentColor"/>
            <circle cx="21" cy="12" r="2" fill="currentColor"/>
            <circle cx="9" cy="10" r="1.5" fill="currentColor"/>
            <circle cx="15" cy="10" r="1.5" fill="currentColor"/>
            <path d="M8 15c1.5 2 6.5 2 8 0" stroke="currentColor" stroke-width="1.5" fill="none" stroke-linecap="round"/>
        </svg>`
        };
        ...
        class StorageService {
                ...
                return {
                    ...
                    settings: {
                        ...
                        panels: {
                            ...
                            right: { collapsed: false, width: 300 },
            ...
            migrate(oldData) {
                ...
                if (oldData.settings) {
                    ...
                    // Preserve existing theme/panels if compatible
                    if (oldData.settings.theme) newData.settings.theme = oldData.settings.theme;
                }

                // Ensure automation settings exist
                if (!newData.settings.automation) {
                    newData.settings.automation = { scenes: 5, timeout: 120, upscale: false, autoDownload: false };
                }
            ...
            save(data) {
                ...
                GM_setValue(DB_KEY, JSON.stringify(this.data));
                // Backup to localStorage for safety
                localStorage.setItem('GPM_V2_MIRROR', JSON.stringify(this.data));
            }
        ...
        class SidePanel extends Component {
            ...
            renderInternal() {
                ...
                <div class="gpm-panel side-panel" style="
                    position: fixed; top: 80px; ${this.side}: 20px;
                    width: ${this.width}px; height: 700px;
            ...
            <div class="sticky-toolbar" style="padding: 10px 10px 0 10px; flex-shrink: 0;">
                ...
                <div class="mode-row" style="display: flex; gap: 6px; margin-bottom: 8px;">
                    ...
                    <button class="gpm-btn auto-loop-btn" title="无限循环 (Infinite Loop)" style="width: 30px; font-size: 14px;">∞</button>
    ...
    `);
            ...
            afterRender() {
                ...
                // Expose visibility methods
                this.show = () => { panel.style.display = 'flex'; };
                this.hide = () => { panel.style.display = 'none'; };
                ...
                // Auto Loop Button
                const autoLoopBtn = this.shadow.querySelector('.auto-loop-btn');
                if (autoLoopBtn) {
                    autoLoopBtn.onclick = () => {
                ...
                minBtn.onclick = () => {
                    ...
                    panel.style.height = this.isCollapsed ? 'auto' : '600px';
            ...
            setup(libraryData, libraries, onPromptClick, onLibChange, onImport, onExport, onAddLib, onDeleteLib, onAddPrompt, onExportAll, onDraftToggle, onPromptAction, onReorder) {
                ...
                if (aiAssist) {
                    aiAssist.onclick = () => {
                        ...
                        window.open('https://grok.com/chat', 'GrokAssist', `width=${width},height=${height},top=${top},left=${left},resizable=yes,scrollbars=yes`);
                    };
                }
            ...
            renderAutomationDashboard(onStart, onStop, settings = {}) {
                const container = this.shadow.querySelector('#prompt-container');
                if (!container) return;
                container.innerHTML = '';

                const defaults = { scenes: 5, timeout: 120, upscale: false, autoDownload: false };
                const cfg = { ...defaults, ...settings };

                // 1. Config Area
                const configHTML = `
                <div style="background: rgba(0,0,0,0.3); padding: 10px; border-radius: 8px; margin-bottom: 10px;">
                    <h3 style="margin: 0 0 10px 0; font-size: 14px; display: flex; align-items: center; gap: 6px;">
                        <span style="font-size: 16px;">♾️</span> Infinite Loop Mode
                    </h3>
                    <div style="display:flex; gap: 8px; margin-bottom: 8px;">
                        <div style="flex:1;">
                            <label style="font-size:10px; color:#888; display:block; margin-bottom:2px;">SCENES</label>
                            <input type="number" id="loop-scenes" value="${cfg.scenes}" style="width:100%;">
                        </div>
                        <div style="flex:1;">
                            <label style="font-size:10px; color:#888; display:block; margin-bottom:2px;">TIMEOUT (s)</label>
                            <input type="number" id="loop-timeout" value="${cfg.timeout}" style="width:100%;">
                        </div>
                    </div>
                    <div style="margin-bottom: 12px; display:flex; flex-direction:column; gap:4px;">
                        <label style="display:flex; align-items:center; gap: 6px; font-size: 12px; cursor: pointer;">
                            <input type="checkbox" id="loop-upscale" ${cfg.upscale ? 'checked' : ''}> ⚡ Auto Upscale (High Quality)
                        </label>
                        <label style="display:flex; align-items:center; gap: 6px; font-size: 12px; cursor: pointer;">
                            <input type="checkbox" id="loop-download" ${cfg.autoDownload ? 'checked' : ''}> 📥 Auto Download
                        </label>
                    </div>
                    <!-- Upload Initial Image -->
                    <div style="margin-bottom: 8px;">
                        <label class="gpm-btn" style="width:100%; text-align:center; padding: 8px; border-style: dashed;">
                            🖼️ Upload Initial Image (Start Frame)
                            <input type="file" id="loop-init-img" accept="image/*" style="display:none;">
                        </label>
                        <div id="loop-img-preview" style="font-size: 11px; color: #888; margin-top: 4px; text-align: center;">Optional</div>
                    </div>
                </div>

                <!-- Status & Controls -->
                <div style="display:flex; gap: 8px; margin-bottom: 10px;">
                    <button class="gpm-btn primary" id="loop-start-btn" style="flex:1; padding: 10px;">▶ RUN LOOP</button>
                    <button class="gpm-btn danger" id="loop-stop-btn" style="flex:1; padding: 10px;" disabled>⏹ STOP</button>
                </div>

                <!-- Log / Queue -->
                <div style="flex:1; display:flex; flex-direction:column;">
                    <div style="font-size:11px; color:#888; margin-bottom:4px;">ACTIVITY LOG</div>
                    <div id="loop-logs" style="
                        background: rgba(0,0,0,0.4); border-radius: 8px; padding: 8px;
                        flex: 1; overflow-y: auto; font-family: monospace; font-size: 11px; color: #ccc;
                        min-height: 200px; border: 1px solid rgba(255,255,255,0.05);
                    ">
                        <div style="color: #666;">Waiting for configuration...</div>
                    </div>
                </div>
            `;
                ...
                // Bindings
                const startBtn = container.querySelector('#loop-start-btn');
                const stopBtn = container.querySelector('#loop-stop-btn');
                const imgInput = container.querySelector('#loop-init-img');
                const imgPreview = container.querySelector('#loop-img-preview');
                const logs = container.querySelector('#loop-logs');

                // Image Preview
                imgInput.onchange = () => {
                    if (imgInput.files[0]) imgPreview.textContent = '✓ ' + imgInput.files[0].name;
                };
                ...
                this.dashboardLog = log; // Expose for external use

                startBtn.onclick = () => {
                    const scenes = parseInt(container.querySelector('#loop-scenes').value) || 5;
                    const timeout = parseInt(container.querySelector('#loop-timeout').value) || 120;
                    const upscale = container.querySelector('#loop-upscale').checked;
                    const initFile = imgInput.files[0];

                    startBtn.disabled = true;
                    stopBtn.disabled = false;
                    logs.innerHTML = ''; // Clear logs
                    log('Initializing loop sequence...', 'info');

                    if (onStart) onStart({ scenes, timeout, upscale, initFile, log });
                };

                stopBtn.onclick = () => {
                    startBtn.disabled = false;
                    stopBtn.disabled = true;
                    log('Stopping loop engine...', 'error');
                    if (onStop) onStop();
                };
            }
...
Breast squeeze, pressing breasts together"></textarea>
        ...
        class BottomPanel extends Component {
            ...
            afterRender() {
                ...
                this.hide = () => {
                    ...
                    // Sync draft back to input
                    if (this.inputManager) {
                        this.inputManager.setValue(this.getText());
                    }
                };
                this.toggle = () => { panel.style.display === 'none' ? this.show() : this.hide(); };
        ...
        /**
         * PromptInspector: Inspects image generation metadata
         */
        class PromptInspector {
            constructor(manager) {
                this.manager = manager;
                this.postStore = GLOBAL_POST_STORE; // Link to global store
            ...
            init() {
                console.log('[Inspector] Initializing UI...');
                this.startObserver();
            }

            // 🔴 FIX #2: Add cleanup method to prevent memory leak
            destroy() {
                if (this.observer) {
            ...
            startObserver() {
                const waitForBody = () => {
                    ...
                    // Observer to watch for new images appearing in the feed
            ...
            injectButtons() {
                // Target the specific card structure in Grok
                const cards = document.querySelectorAll('[role="listitem"] .group\\/media-post-masonry-card');

                cards.forEach(card => {
                    if (card.querySelector('.pi-inspector-btn')) return;

                    const media = card.querySelector('img, video');
                    if (!media) return;

                    const src = media.src || media.poster;
                    if (!src || this.isPublicImage(src)) return;
            ...
            createButton() {
                const btn = document.createElement('button');
                btn.className = 'pi-inspector-btn';
                btn.textContent = '✦'; // Sparkle icon
                ...
                btn.addEventListener('mouseenter', () => {
                    btn.style.background = 'rgba(29, 155, 240, 0.9)'; // Brand color
            ...
            isPublicImage(src) {
                // Filter out public/feed images if necessary, or refine this logic
                return src && src.includes('xai-images-public');
            }
            ...
            extractPostId(card) {
                ...
                // Strategy 1: Grok assets URL structure
            ...
            openPanel(card) {
                try {
                    ...
                    const post = this.postStore[postId];

                    const panel = this.createPanel(post, postId);
                    panel.addEventListener('mouseenter', () => clearTimeout(this.hoverTimer));
                    panel.addEventListener('mouseleave', () => this.scheduleClose());

                    card.appendChild(panel);
                    this.activePanel = panel;
                } catch (e) {
                    console.warn('[Inspector] Open panel failed:', e);
                }
            }

            createPanel(post, postId) {
                const panel = document.createElement('div');
                panel.className = 'pi-inspector-panel';

                let html = `<div style="padding-bottom: 8px; border-bottom: 1px solid rgba(255,255,255,0.1); margin-bottom: 8px;">
                <strong style="font-size:13px;">🔍 Prompt Inspector</strong>
            </div>`;

                if (!post) {
                    html += `<div style="color:#888;font-size:12px; padding: 10px; text-align: center;">
                    Waiting for data...<br>
                    <span style="font-size: 10px; opacity: 0.7;">(Try scrolling or refreshing if persistent)</span>
                </div>`;
                } else {
                    html += this.renderPostInfo(post);
                }
            ...
            renderPostInfo(post) {
                ...
                // Metadata
                html += `
                <div style="font-size:11px; color:#aaa; display: grid; grid-template-columns: 1fr 1fr; gap: 4px;">
                    <div><b>Model:</b> ${post.modelName || 'Grok 2'}</div>
                    <div><b>Time:</b> ${formatTime(post.createTime)}</div>
                </div>
            `;

                // Function to render a prompt block
                const renderBlock = (label, text, type) => `
                <div style="background: rgba(255,255,255,0.05); border-radius: 8px; padding: 8px;">
                    <div style="font-weight: bold; font-size: 11px; margin-bottom: 4px; color: var(--gpm-primary, #1d9bf0);">
                        ${label}
                    </div>
                    <div style="white-space: pre-wrap; word-break: break-word; line-height: 1.4; color: #eee;">${text}</div>
                     <div style="display:flex; gap:6px; margin-top:8px;">
                        <button data-copy="${encodeURIComponent(text)}" class="gpm-btn">📋 Copy</button>
                        <button data-save="${encodeURIComponent(text)}" class="gpm-btn">💾 Save</button>
                    </div>
                </div>
            `;

                // User Input
                if (post.originalPrompt) {
                    html += renderBlock('User Input', post.originalPrompt, 'user');
                }

                // Enhanced/Rewritten Prompt (if different)
                if (post.prompt && post.prompt !== post.originalPrompt) {
                    html += renderBlock('Enhanced Prompt', post.prompt, 'enhanced');
                }

                // History/Chain
                if (post.childPosts && post.childPosts.length > 0) {
                    html += `<div style="font-size: 11px; opacity: 0.7; margin-top: 4px;">History: ${post.childPosts.length} versions</div>`;
                }
            ...
            bindPanelEvents(panel) {
                ...
                // Save buttons
                panel.querySelectorAll('button[data-save]').forEach(btn => {
                    btn.onclick = (e) => {
                        e.stopPropagation();
                        const text = decodeURIComponent(btn.dataset.save);
                        // Emit custom event for App to handle
                        window.dispatchEvent(new CustomEvent('gpm-save-prompt', {
                            detail: { text: text, source: 'inspector' }
                        }));
        ...
        class App {
            ...
            init() {
                ...
                // Mount Panels
                this.leftPanel = new SidePanel(this.styles, { side: 'left', width: 300 });
                this.leftPanel.renderInternal();
                this.leftPanel.mount(this.container);

                this.rightPanel = new SidePanel(this.styles, { side: 'right', width: 300 });
                ...
                this.inspector = new PromptInspector(this);
                this.inspector.init();

                // 🔴 FIX #2 (continued): Bind cleanup to page unload
                ...
                // Listen for save events from Inspector
                window.addEventListener('gpm-save-prompt', (e) => {
                    this.saveFromInspector(e.detail.text);
                });
            ...
            addNewPrompt(content, type = 'text') {
                const data = this.storage.get();
                ...
                if (activeLib) {
                    // Smart Title Logic
                    let title = content.slice(0, 20) + (content.length > 20 ? '...' : ''); // Default
            ...
            onToggleAutomation() {
                // Switch View State
                if (this.currentView === 'automation') {
                    this.currentView = 'list';
                    this.loadLibraryData(); // Renders list
                } else {
                    this.currentView = 'automation';
                    // Get right panel (Video)
                    const panel = this.panels.right;
                    if (panel) {
                        panel.renderAutomationDashboard(
                            (cfg) => this.handleAutomationStart(cfg),
                            () => this.handleAutomationStop()
                        );
                    }
                }
            }

            async handleAutomationStart(config) {
                const { scenes, timeout, upscale, initFile, log } = config;
                this.automation.isRunning = true;
                let lastFrameBlob = initFile ? initFile : null; // Start with user file if any

                // Get Prompts to Loop (Use active library prompts as a source)
                const data = this.storage.get();
                const activeLib = data.libraries.find(l => l.id === data.activeVideoLibraryId);
                const prompts = activeLib ? activeLib.prompts.filter(p => p.type !== 'video' || true) : [];
                // Simple strategy: Cycle through prompts
                let promptIndex = 0;

                try {
                    // Cache existing videos to detect new ones
                    const existingVideos = new Set(Array.from(document.querySelectorAll('video')).map(v => v.src));

                    for (let i = 1; i <= scenes; i++) {
                        if (!this.automation.isRunning) {
                            log('Loop stopped by user.', 'warn');
                            break;
                        }

                        log(`🎬 Scene ${i}/${scenes} started...`);

                        // 1. Prepare Prompt
                        const prompt = prompts[promptIndex % prompts.length]?.content || "A creative continuation of the video, high quality, 4k";
                        promptIndex++;

                        // 2. Input Prompt
                        log(`⌨️ Typing prompt...`);
                        const inputEl = this.inputManager.getInput();
                        if (!inputEl) throw new Error("Input field not found!");

                        // Human Type
                        await this.automation.typeHumanly(inputEl, prompt);

                        // 3. Upload Image (Init file or Last Frame)
                        if (lastFrameBlob) {
                            log(`🖼️ Uploading reference image...`);
                            await this.automation.uploadImage(lastFrameBlob);
                        } else if (i > 1 && !lastFrameBlob) {
                            log(`⚠️ No previous frame found, proceeding text-only...`, 'warn');
                        }
                        ...
                        // 5. Wait for Result
                        log(`⏳ Waiting for generation (${timeout}s)...`);
                        const newVideoUrl = await this.automation.waitForNewVideo(existingVideos, timeout * 1000);
                        log(`✅ Video generated: ${newVideoUrl.slice(0, 50)}...`, 'success');

                        existingVideos.add(newVideoUrl); // Add to known set

                        // 5.5 Auto Upscale
                        if (upscale) {
                            log(`✨ Triggering Auto Upscale...`);
                            try {
                                const upscaledUrl = await this.automation.triggerUpscale(newVideoUrl);
                                if (upscaledUrl) {
                                    log(`✅ Upscale successful!`, 'success');
                                    newVideoUrl = upscaledUrl;
                                    existingVideos.add(newVideoUrl);
                                } else {
                                    log(`⚠️ Upscale button/result not found.`, 'warn');
                                }
                            } catch (err) {
                                log(`❌ Upscale Error: ${err.message}`, 'error');
                            }
                        }

                        // 6. Extract Frame for Next Loop
                        if (i < scenes) {
                            log(`📸 Extracting last frame...`);
                            await this.automation.wait(2000); // Give it a sec to stabilize
                            lastFrameBlob = await this.automation.extractLastFrame(newVideoUrl);
                            log(`✨ Frame extracted (${(lastFrameBlob.size / 1024).toFixed(1)}KB)`);
                        }

                        // Cooldown
                        if (i < scenes) {
                            log(`zzz Cooling down (5s)...`);
                            await this.automation.wait(5000);
                        }
                    }

                    log('🎉 Infinite Loop Sequence Completed!', 'success');
                    ...
                    // Re-enable start button is handled by UI logic if we had a callback, 
                    // but simple way is just let user click stop/start or we re-render.
                    // Ideally we should call a "finished" callback on dashboard.
                    log('🏁 Session Ended.');
                }
            }
            ...
            loadLibraryData() {
                ...
                if (videoLibrary) {
                    ...
                    // Render Modifiers (Video Panel) - Sync with Text Panel modifiers or use separate?
                    // User asked for sync, so we use the same data.modifiers
                    this.rightPanel.renderModifiers(
                        data.modifiers || [],
                        (newMod) => this.addModifier(newMod),
                        (modToDelete) => this.deleteModifier(modToDelete),
                        (mod) => this.handleModifierClick(mod),
                        () => this.importModifiers(),
                        () => this.exportModifiers()
                    );
                }
                ...
                input.onchange = (e) => {
                    ...
                    reader.onload = (evt) => {
                        try {
                            ...
                            let count = 0;
                            json.forEach(mod => {
                                if (typeof mod === 'string' && !data.modifiers.includes(mod)) {
            ...
            exportModifiers() {
                ...
                const a = document.createElement('a');
                a.href = url;
                a.download = `modifiers_backup_${new Date().toISOString().slice(0, 10)}.json`;
            ...
            deleteModifier(mod) {
                const data = this.storage.get();
                if (data.modifiers) {
                    data.modifiers = data.modifiers.filter(m => m !== mod);
                    this.storage.save(data);
                    this.loadLibraryData();
                }
            }
            ...
            handleModifierClick(mod, isReplace = false) {
                if (isReplace) {
                    this.bottomPanel.setText(mod);
                } else {
                    this.bottomPanel.insertText(mod);
                }
            ...
            saveFromInspector(text) {
                // Find default library or active one
                const data = this.storage.get();
                const libId = data.activeLibraryId;
                const libIndex = data.libraries.findIndex(l => l.id === libId);

                if (libIndex === -1) return;

                const newPrompt = {
                    id: Date.now(),
                    name: 'Inspect: ' + text.slice(0, 15) + '...',
                    content: text,
                    category: 'Inspector',
                    pinned: false,
                    type: 'text'
                ...
                data.libraries[libIndex].prompts.unshift(newPrompt);
                this.storage.save(data);
                this.loadLibraryData(); // Refresh UI

                // Optional: Show simple toast? For now just log
                console.log('[GPM] Saved prompt from inspector');
            }
        ...
        window.showGPMPasteImport = function (cb) {
            ...
            <div style="width:600px;background:#202020;border:1px solid rgba(255,255,255,0.2);border-radius:12px;padding:20px;color:white;display:flex;flex-direction:column;gap:15px;box-shadow:0 10px 40px rgba(0,0,0,0.5);">
                ...
                <div style="color:#888;font-size:12px;">Supported: Name[tab/space]Content OR Just Content</div>
