...
// @version      0.18
// @description  Grok Imagine 提示词管理工具 (Grok Prompt Manager)
// @author       You
...
(function () {
    ...
    class GrokPromptManager {
        constructor() {
            ...
            // New Data Model:
            // libraries: [ { id: 'default', name: '📚 默认库 (Default)', prompts: { text_to_image: [], image_to_video: [] } } ]
            this.libraries = [];
        ...
        updateBottomPanel() {
            ...
            this.bottomPanel.innerHTML = `
                <div class="gpm-bp-header">
                    <div class="gpm-bp-tab ${isDraft ? 'active' : ''}" data-tab="draft">📝 草稿台 (Draft)</div>
                    <div class="gpm-bp-tab ${!isDraft ? 'active' : ''}" data-tab="history">📜 历史 (History)</div>
                    <div class="gpm-bp-close">×</div>
                </div>
                <div class="gpm-bp-content">
                    ${isDraft ? `
                        <textarea id="sp-text" class="gpm-sp-textarea" placeholder="在此组合、编辑提示词...">${this.draftContent}</textarea>
                        <div class="gpm-sp-actions">
                            <button id="sp-clear" class="gpm-btn">清空</button>
                            <button id="sp-copy" class="gpm-btn">复制</button>
                            <button id="sp-send" class="gpm-btn primary">发送到 Grok</button>
                        </div>
                    ` : `
                        <div id="history-list" class="gpm-hist-list"></div>
                        <div class="gpm-sp-actions" style="margin-top:8px">
                             <button id="hist-clear" class="gpm-btn danger">清空历史</button>
                        </div>
                    `}
                </div>
            `;
            ...
            if (isDraft) {
                ...
                this.bottomPanel.querySelector('#sp-copy').onclick = () => { navigator.clipboard.writeText(spText.value); this.showToast('已复制草稿'); };
                ...
                this.bottomPanel.querySelector('#hist-clear').onclick = () => {
                    if (confirm('清空历史？')) { this.history = []; this.saveData(); this.renderHistoryList(); }
                };
            }
        }
        ...
        renderHistoryList() {
            ...
            if (!list) return;
            list.innerHTML = '';
            if (this.history.length === 0) { list.innerHTML = '<div style="text-align:center;color:#666;padding:20px;">无历史记录</div>'; return; }
